...
def load_X_Y(dataset, split, encoding_name):
    ...
    X = [i['x_image'] for i in stream_json(str(json_file))]
    Y = [i['x_text'] for i in stream_json(str(json_file))]
...
def cca(dataset, encoding_name):
    ...
    param_grid = {
        'n_components': [30, 40, 50, 60],
        'center': [True],
        'reg': [1e-3, 2.5e-3, 5e-3, 7.5e-3, 1e-2],
        'regscaled': [True],
        'scale_by_eigs': [True],
        'norm': [True, False]
    }
    ...
    for params in ParameterGrid(param_grid):
        ...
        results.append((params, benchmark_func(None, 'flickr8k', 'validation', X_c, Y_c)))
